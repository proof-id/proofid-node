...
pub mod pallet {
	use super::*;
	use pallet_ctype::CtypeHashOf;
	use pallet_delegation::DelegationNodeIdOf;
	use frame_support::{
		pallet_prelude::*,
	...
	pub(crate) type AttesterOf<T> = pallet_delegation::DelegatorIdOf<T>;
	...
	#[pallet::config]
	pub trait Config: frame_system::Config + pallet_ctype::Config + pallet_delegation::Config {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add(
			...
			let deposit_amount = <T as crate::Config>::Deposit::get();
				...
				<pallet_ctype::Ctypes<T>>::contains_key(&ctype_hash),
				pallet_ctype::Error::<T>::CTypeNotFound
			);
			ensure!(
			...
			let delegation_record = if let Some(delegation_id) = delegation_id {
				let delegation = <pallet_delegation::DelegationNodes<T>>::get(delegation_id)
					...
					(delegation.details.permissions & pallet_delegation::Permissions::ATTEST)
				...
				let root = <pallet_delegation::DelegationHierarchies<T>>::get(delegation.hierarchy_root_id)
					.ok_or(pallet_delegation::Error::<T>::HierarchyNotFound)?;
	...
	impl<T: Config> Pallet<T> {
		...
		fn verify_delegated_access(
			...
			let (is_delegating, delegation_depth) =
				<pallet_delegation::Pallet<T>>::is_delegating(attester, &delegation_id, max_parent_checks)?;
