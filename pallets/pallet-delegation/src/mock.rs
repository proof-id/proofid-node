...
use pallet_ctype::mock as ctype_mock;
use frame_support::{
	parameter_types,
...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		Ctype: pallet_ctype::{Pallet, Call, Storage, Event<T>},
...
impl pallet_ctype::Config for Test {
	...
	T::Hash: From<H256>,
{
	DelegationHierarchyCreationOperation {
		id,
		ctype_hash: pallet_ctype::mock::get_ctype_hash::<T>(true),
	}
}
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		ext.execute_with(|| {
			for (ctype_hash, owner) in self.ctypes.iter() {
				pallet_ctype::Ctypes::<Test>::insert(ctype_hash, owner);
			}
